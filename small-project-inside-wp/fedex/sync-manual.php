...
try {
    ...
    if (count($shipped_orders)) {
        ...
        echo sprintf("Tracking number of %s new shipped orders are synced.", count($shipped_orders));
    } else {
        echo "No new shipped orders found to sync tracking number.";
    }
} catch (Exception $e) {
    mail('moh.mainul.hasan@gmail.com', 'WP cron error', $e->getMessage(), 'From: info@wp_site_name.com');
